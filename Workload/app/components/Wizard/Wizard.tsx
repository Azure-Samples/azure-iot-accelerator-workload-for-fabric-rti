...
import { completeWizardSafely, getPersistableWizardContext } from "./wizardUtils";
        ...
        <div className="wizard-control__navigation">
            ...
            <div className="wizard-control__navigation-right">
                <Button 
                    appearance="secondary"
                    onClick={onPrevious}
                    disabled={disablePrevious || navigationDisabled || isCompleting}
                ...
                {isLastStep ? (
                    <Button 
                        appearance="primary"
                        onClick={onComplete}
                        disabled={!canFinish || navigationDisabled || isCompleting}
                        data-testid="wizard-complete-btn"
                    >
                        {isCompleting ? "Saving..." : labels.complete}
                    </Button>
                ) : (
                    <Button 
                        appearance="primary"
...
export interface WizardStep {
    ...
    description?: string;
    /** Whether this page participates in setup-step numbering (default: true) */
    numbered?: boolean;
    ...
    /** Whether navigation should be disabled while this step is performing work */
    isNavigationDisabled?: (context: Record<string, any>) => boolean;
...
export interface WizardStepProps {
    ...
    /** Reset completed status for steps starting at the given index */
    resetStepsFrom: (fromIndex: number) => void;
...
export interface WizardControlProps {
    ...
    /** Callback fired when Previous is clicked on the first step. If provided, Previous is enabled on step 1. */
    onFirstStepBack?: () => void;
    /** Key for persisting wizard state in sessionStorage. If set, context, step, and completed steps survive remounts. */
    persistKey?: string;
}
...
export interface WizardNavigationProps {
    ...
    canFinish: boolean;
    /** Whether the current step passes validation (for disabling Next) */
    canGoNext: boolean;
    /** Whether the Previous button should be disabled */
    disablePrevious: boolean;
    /** Whether all navigation should be disabled by the current step */
    navigationDisabled: boolean;
    /** Whether the wizard is currently completing (saving) */
...
export function WizardControl({ 
    ...
    const persisted = React.useMemo(loadPersisted, [persistKey]);

    // State management — restore from sessionStorage if available
    const [currentStepId, setCurrentStepId] = React.useState(
        persisted?.currentStepId || initialStepId || steps[0]?.id
    );
    const [wizardContext, setWizardContext] = React.useState<Record<string, any>>(
        persisted?.wizardContext || initialContext
    );
    const [completedSteps, setCompletedSteps] = React.useState<Set<string>>(
        new Set(persisted?.completedSteps || [])
    );
    ...
    const navigationDisabled = !!currentStep?.isNavigationDisabled?.(wizardContext);
    ...
    React.useEffect(() => {
        ...
        try {
            sessionStorage.setItem(persistKey, JSON.stringify({
                currentStepId,
                wizardContext: getPersistableWizardContext(wizardContext),
                completedSteps: Array.from(completedSteps),
            }));
        } catch { /* quota exceeded — ignore */ }
    }, [persistKey, currentStepId, wizardContext, completedSteps]);
    ...
    // Track whether the current step passes validation (for disabling Next)
    const [canGoNext, setCanGoNext] = React.useState(false);
    React.useEffect(() => {
        let cancelled = false;
        const check = async () => {
            if (!currentStep?.validate) {
                if (!cancelled) setCanGoNext(true);
                return;
            }
            try {
                const valid = await currentStep.validate(wizardContext);
                if (!cancelled) setCanGoNext(valid);
            } catch {
                if (!cancelled) setCanGoNext(false);
            }
        };
        check();
    ...
    const completeWizard = async () => {
        ...
        setCompletionError("");
        const error = await completeWizardSafely(onComplete, wizardContext);
        if (error) {
    ...
    /**
     * Resets completed status for steps at and after the given index
     */
    const resetStepsFrom = React.useCallback((fromIndex: number) => {
    ...
    const stepProps: WizardStepProps = {
        ...
        onGoToStep: goToStep,
        resetStepsFrom,
        isFirstStep: currentStepIndex === 0 && !onFirstStepBack,
    ...
    const navigationProps: WizardNavigationProps = {
        ...
        canGoNext,
        disablePrevious: currentStepIndex === 0 && !onFirstStepBack,
        navigationDisabled,
        isCompleting,
        onPrevious: handlePrevious,
        onNext: handleNext,
    ...
    return (
        <div className={`wizard-control ${className}`.trim()}>
            ...
            {completionError && (
                <div role="alert" className="wizard-control__completion-error">
                    {completionError}
                </div>
            )}
                ...
                <div className="wizard-control__steps-panel">
                    <div>
                        {steps.length > 1 && steps.map((step, index) => {
                            ...
                            const isNumbered = step.numbered !== false;
                            const displayStepNumber = steps
                                .slice(0, index + 1)
                                .filter(candidate => candidate.numbered !== false)
                                .length;
                                        ...
                                        <div className={`wizard-control__step-circle wizard-control__step-circle--${status}`}>
                                            {status === 'completed' ? (
                                                ...
                                                <Text 
                                                    ...
                                                    aria-label={`Step ${displayStepNumber}`}
                                                >
                                                    {displayStepNumber}
                                                </Text>
                                            ) : (
                                                <span
                                                    className={`wizard-control__page-marker wizard-control__page-marker--${status}`}
                                                    aria-hidden="true"
                                                />
                                            )}
                                        </div>
